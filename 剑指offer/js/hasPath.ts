 ...
 */
/**********/
function hasPath(matrix: string[], rows: number, cols: number, path: string[]): boolean
{
    if ((matrix instanceof Array && matrix.length === 0) || rows === 0 || cols === 0)   return false;
    if (path instanceof Array && path.length === 0)    return true;

    let pathLength = 0;
    const flagMatrix = new Array(matrix.length).fill(false);
...
function hasPathCore(
    matrix: string[], rows: number, cols: number, row: number, col: number,
    str: string[], pathLength: number, flagMatrix: boolean[]): boolean {
